...
struct RitualCompletionView: View {
    ...
    init(intensity: Intensity, completedAt: Date, palette: ThemePalette = .quietIvory, onRestart: @escaping () -> Void, onRecords: (() -> Void)? = nil) { self.intensity = intensity; self.completedAt = completedAt; self.palette = palette; self.onRestart = onRestart; self.onRecords = onRecords }
    var body: some View {
